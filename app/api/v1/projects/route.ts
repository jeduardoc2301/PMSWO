    ...
    return NextResponse.json(
      {
        projects: result.projects.map((project) => ({
          ...
          overdueWorkItems: (project as any).overdueWorkItems ?? 0,
          criticalBlockers: (project as any).criticalBlockers ?? 0,
          highRisks: (project as any).highRisks ?? 0,
          healthCategory: (project as any).healthCategory ?? 'SIN_ALERTAS',
          spi: (project as any).spi ?? null,
          timeElapsedPct: (project as any).timeElapsedPct ?? 0,
          scheduleVariance: (project as any).scheduleVariance ?? null,
