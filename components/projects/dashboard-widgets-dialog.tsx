  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div
        ...
        className="w-full max-w-lg rounded-xl border border-zinc-800 bg-[#18181b] p-6"
      >
        <div className="mb-4 flex items-baseline justify-between gap-3">
          <h2 className="text-base font-medium text-zinc-100">Configurar widgets</h2>
          <button
            type="button"
            aria-label="Cerrar"
            onClick={onCerrar}
        ...
        <ul className="flex flex-col gap-1">
          {CATALOGO.map((widget) => {
            ...
            return (
              <li key={widget.clave}>
                <label className="flex cursor-pointer items-start gap-3 rounded-lg px-2 py-2.5 hover:bg-zinc-800/60">
                  <input
                    type="checkbox"
                    checked={encendido}
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm text-zinc-100">{widget.nombre}</span>
                    <span className="block text-xs text-zinc-500">{widget.detalle}</span>
                    {widget.sinDatos ? (
        ...
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            ...
            className="rounded-lg bg-[#6366f1] px-4 py-2 text-sm font-medium text-white hover:bg-[#5457e5] disabled:opacity-50"
          >
